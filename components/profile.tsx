...
export default function UserProfile() {
    ...
    const mapping = { "Projects": toggleProjectSection, "Work Experience": toggleWorkSection, "Education": toggleEducationSection, "Extracurriculars": toggleExtraSection };
                        ...
                        <div className="absolute right-0 mt-10 w-48 bg-white border rounded shadow-lg">
                            <ul className="py-2">
                                {Object.entries(mapping).map(([key, value]) => (
                                    <DropdownButton key={key} toggleSection={() => value()} toggleDropdown={toggleDropdown} text={key} />
                                ))}
                            </ul>
                        </div>
                    )}
